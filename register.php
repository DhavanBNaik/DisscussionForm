...
if(isset($_POST['submit'])){
   ...
   if(mysqli_num_rows($select) > 0){
      ...
      elseif(preg_match($emailpattern,$email) == 0){
         $message[]='Please register through college mail id';
      }
      // elseif($image_size > 2000000){
      //    $message[] = 'image size is too large!';
      // }
      else{
         $insert = mysqli_query($conn, "INSERT INTO `user_form`(name,usn,email, password)
                                 VALUES('$name','$usn','$email', '$pass')")
                                 or
                                 die('query failed');
